...
- [ ] Project scaffolding: `seed-app/backend/` with FastAPI, requirements.txt, app structure
- [ ] Database layer: SQLAlchemy 2.0 engine, session factory, Base model (`database.py`)
- [ ] Domain models:
...
- [ ] Auth system: mock JWT login (`POST /api/auth/login` — email lookup, returns token with member_id)
- [ ] Auth dependency: `get_current_member` extracts member from token
- [ ] Routes + Services:
...
- [ ] Auto-seed on startup (create tables + populate if empty)
- [ ] Health check endpoint: `GET /api/health`

### Phase 1B: Frontend Foundation

- [ ] Project scaffolding: `seed-app/frontend/` with Vite + React 18 + Tailwind
- [ ] Tailwind config with healthcare design tokens:
...
- [ ] `useApi` hook — wraps fetch with Authorization header, base URL, error handling
- [ ] Layout:
...
- [ ] React Router v6 setup with layout outlet pattern
- [ ] API proxy config in Vite (proxy `/api` to `:8000`)

### Phase 1C: Integration & Polish

- [ ] `seed-app/README.md` — patterns guide for agents: file structure, how to add a route, how to add a page, naming conventions, database patterns
- [ ] Makefile targets: `make seed-backend`, `make seed-frontend`, `make dev`
- [ ] docker-compose.yml (optional for Phase 1 — can use Makefile with subprocesses)
- [ ] End-to-end verification: login → dashboard → plan → medications all work
...
- [ ] Project scaffolding: `orchestrator/` with FastAPI + requirements
- [ ] Task state machine: QUEUED → ASSIGNED → IN_PROGRESS → DONE | FAILED
- [ ] DAG definition: declare 6 tasks with dependency edges (PM→TL→Arch→[BE,FE]→QA)
- [ ] Async DAG executor: polls for tasks whose deps are satisfied, runs them
- [ ] WebSocket endpoint: `/ws` — clients connect, receive all events
- [ ] Event bus: publish events `{ type, agent, data, timestamp }`
  - Event types: `task_state_change`, `agent_output`, `tool_call`, `file_write`, `test_result`, `pipeline_start`, `pipeline_complete`
- [ ] Mock agent runner: simulates each agent with canned output + realistic delays
- [ ] REST endpoints:
...
- [ ] Workspace management: create temp directory, track files
- [ ] Agent tool definitions (interfaces only, wired in Phase 4):
